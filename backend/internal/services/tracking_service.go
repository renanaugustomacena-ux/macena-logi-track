...
func (t *TrackingService) LatestPosition(ctx context.Context, tenantID, shipmentID string) (*logistics.Waypoint, error) {
...
func (t *TrackingService) DistanceRemainingKM(current, destination logistics.GeoPoint) float64 {
...
func (t *TrackingService) IsDelayed(s logistics.Shipment, threshold time.Duration, now time.Time) bool {
	if s.Status == logistics.StatusDelivered || s.Status == logistics.StatusCancelled {
		return false
	}
	if s.ETA.IsZero() {
		return false
